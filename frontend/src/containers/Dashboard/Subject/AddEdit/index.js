...
const AddSubjectContainer = (props) => {
    ...
    useEffect(() => {
        ...
        if (!unmount) {
            setState({
                ...props,
                ...props.data,
                subCategory: props.data?.subCategory?._id,
    ...
    const updateSubject = async (data) => {
        try {
            const response = await Axios.put(`${API_URL}/subjects/${state._id}`, data, {
                headers: {
    ...
    const csvUploadProps = {
        name: 'file',
        action: `${API_URL}/subjects/import`,
    ...
    const { title, description, display, subCategories, image, subCategory } = state;
    return (
        <Drawer
            title={props.edit ? "Edit subject" : "Add new subject"}
            width={600}
            style={{
                maxWidth: '100%'
            }}
        ...
        >
                ...
                <Row gutter={[16, 32]}>
                    <Col xs={24}>
                        <Tooltip title="Import from csv. Also copy subCategory id from table for reference, add it to csv under subCategory.">
                            <Upload
                ...
                <Row gutter={16, 32}>
                    ...
                    <Col span={24}>
                        <Form.Item
                            name="subCategory"
                            label="Sub Category"
                            rules={[{ required: false, message: 'Select sub category' }]}
                        >
                            <Select
                                name="category"
